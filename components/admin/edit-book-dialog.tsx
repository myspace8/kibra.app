...
export function EditBookDialog({ open, onOpenChange, book }: EditBookDialogProps) {
  ...
  const [coverPreview, setCoverPreview] = useState<string | null>(book.cover_image_url)
  const [pdfPreview, setPdfPreview] = useState<string | null>(book.pdf_url.split("/").pop() || null)
  ...
  // Reset form and previews when the book prop changes
  useEffect(() => {
    if (open) {
      form.reset({
        title: book.title,
  ...
  const onSubmit = async (data: BookFormValues) => {
    ...
    try {
      ...
      formData.append("existing_cover_image_url", book.cover_image_url || "")
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Update the details below to edit this book.</DialogDescription>
        </DialogHeader>
